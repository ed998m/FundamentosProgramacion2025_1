...
// Mapa de calor con colores ANSI
void drawHeatMap(double* T, int n, int c) {
    ...
    printf("\nMapa de calor (colores ANSI):\n");
    for (int i = 0; i < n; i++) {
        double norm = (T[i] - min) / (max - min);

        if (norm < 0.2) printf("\033[48;5;21m  \033[0m");      // Azul oscuro
        else if (norm < 0.4) printf("\033[48;5;33m  \033[0m");  // Azul medio
        else if (norm < 0.6) printf("\033[48;5;226m  \033[0m"); // Amarillo
        else if (norm < 0.8) printf("\033[48;5;202m  \033[0m"); // Naranja
        else printf("\033[48;5;196m  \033[0m");                 // Rojo fuerte
...
int main() {
    ...
    // Eliminar conexiones cruzadas entre filas
    for (int i = 1; i < c; i++) {
        int row = i * c - 1;
        int col = i * c;
        A[row][col] = 0.0;
